...
public class ReformatCodeProcessor extends AbstractLayoutCodeProcessor {
  ...
  public ReformatCodeProcessor(@NotNull PsiFile file, ReformatScope reformatScope) {
        ...
        () ->
            SlowOperations.allowSlowOperations(
                () -> {
                  ...
                  scheduleMavenRun(spotlessApply, settings);
                }));
  }

  private void scheduleMavenRun(
      final MavenRunnerParameters params, final MavenRunnerSettings settings) {
    MavenRunConfigurationType.runConfiguration(myProject, params, null, settings, null, true);
  }

  private MavenRunnerSettings constructSettingsWithFileToProcessArgument(
      final PsiFile fileToProcess) {
    final MavenRunnerSettings mavenRunnerSettings = new MavenRunnerSettings();
    if (reformatScope == ReformatScope.FILE) {
      mavenRunnerSettings.setMavenProperties(
          Map.of("spotlessFiles", fileToProcess.getVirtualFile().getPath()));
    }
    mavenRunnerSettings.setRunMavenInBackground(true);
    return mavenRunnerSettings;
  }

  private MavenRunnerParameters constructRunnerParametersWithSpotlessApplyGoal(
      final PsiFile fileToProcess) {
    ...
    return new MavenRunnerParameters(
        true,
        fileToProcess.getVirtualFile().getParent().getPath(),
        pomPath,
